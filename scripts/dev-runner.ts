...
/**
 * `role` matters because only the backend honours `--host`/`MARCODE_HOST`; the
...
export class DevRunnerInvalidPortOffsetError extends Schema.TaggedErrorClass<DevRunnerInvalidPortOffsetError>()(
  ...
  {
    configKey: Schema.Literal("MARCODE_PORT_OFFSET"),
...
> {
  ...
  if (seed) {
    if (/^\d+$/.test(seed)) {
      return Effect.succeed({
        offset: Number(seed),
        source: `numeric MARCODE_DEV_INSTANCE=${seed}`,
      });
    }

    const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
    return Effect.succeed({ offset, source: `hashed MARCODE_DEV_INSTANCE=${seed}` });
  }
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    if (configuredBaseDir !== undefined) {
      output.MARCODE_HOME = resolvedBaseDir;
    } else {
      delete output.MARCODE_HOME;
    }
    ...
    if (!isDesktopMode) {
      ...
      if (mode === "dev" || mode === "dev:web") {
        ...
        output.MARCODE_SINGLE_ORIGIN_DEV = "1";
      } else {
        output.VITE_HTTP_URL = `http://localhost:${serverPort}`;
        output.VITE_WS_URL = `ws://localhost:${serverPort}`;
        delete output.MARCODE_SINGLE_ORIGIN_DEV;
      }
    } else {
      output.MARCODE_PORT = String(serverPort);
    ...
    if (!isDesktopMode) {
      output.MARCODE_NO_BROWSER = browser === true ? "0" : "1";
    }
 ...
 * target reach it there. When `--host`/`MARCODE_HOST` moves the backend onto
  ...
  return Effect.gen(function* () {
      ...
      Effect.mapError(
        (cause) =>
          new DevRunnerConfigurationError({
            configKeys: ["MARCODE_PORT_OFFSET", "MARCODE_DEV_INSTANCE"],
            cause,
          }),
      ),
    );
    ...
    const selectionSuffix =
      serverOffset !== offset || webOffset !== offset
        ...
        : "";
    const baseDir = env.MARCODE_HOME ?? (yield* DEFAULT_T3_HOME);

    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.MARCODE_PORT)} webPort=${String(env.PORT)} baseDir=${baseDir}`,
    );
    ...
    if (input.share) {
      if (input.mode === "dev:server") {
        ...
        if (shared) {
          ...
          env.MARCODE_DEV_ALLOWED_ORIGINS = [
            env.MARCODE_DEV_ALLOWED_ORIGINS,
          ...
          // a few chunk requests. Only defaulted, so MARCODE_BUNDLED_DEV=0
          // still opts a --share run back out.
          if (env.MARCODE_BUNDLED_DEV === undefined) {
            env.MARCODE_BUNDLED_DEV = "1";
          }
