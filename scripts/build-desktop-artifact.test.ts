  ...
  it.effect("applies platform-specific packaging to the build config", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(mac.dmg, {
        // Marcode's desktop display name, not upstream's "T3 Code (Alpha)". The
        // Linux protocol assertion below deliberately keeps the upstream-shaped
        // `t3code` schemes, which OAuth deep links still depend on.
