// @effect-diagnostics nodeBuiltinImport:off - Static assertions read checked-in DMG source files, and packaged-archive fixtures compute the sidecar digest with the same Node primitive as the builder.
...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("switches desktop packaging product names to nightly for nightly builds", () => {
    assert.equal(resolveDesktopProductName("0.0.17"), "Marcode");
    assert.equal(resolveDesktopProductName("0.0.17-nightly.20260413.42"), "Marcode (Nightly)");
  });

  it("enables passkey signing only when Marcode passkey configuration is explicit", () => {
    ...
    assert.isTrue(
      hasMacPasskeySigningConfiguration({
        MARCODE_APPLE_TEAM_ID: "7C5H84NP44",
      }),
    );
  });
        ...
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                MARCODE_DESKTOP_UPDATE_REPOSITORY: "maarco/marcode",
              },
            }),
          ),
        ),
      ...
      const nightlyConfig = yield* resolveGitHubPublishConfig("nightly").pipe(
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                GITHUB_REPOSITORY: "maarco/marcode",
              },
            }),
          ),
        ),
    ...
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const fixtureDir = yield* fs.makeTempDirectoryScoped({
          prefix: "marcode-mac-icon-matte-",
        });
        const stageResourcesDir = path.join(fixtureDir, "resources");
        yield* fs.makeDirectory(stageResourcesDir, { recursive: true });

        const source = new PNG({ width: 2, height: 1 });
        // fully transparent pixel, then an opaque black pixel
        source.data.set([0, 0, 0, 0, 0, 0, 0, 255]);
        const sourcePng = path.join(fixtureDir, "black-macos-1024.png");
        yield* fs.writeFile(sourcePng, new Uint8Array(PNG.sync.write(source)));

        const commands: Array<{ readonly command: string; readonly args: ReadonlyArray<string> }> =
          [];
        yield* stageMacIcons(stageResourcesDir, sourcePng, false).pipe(
          Effect.provide(iconResizeSpawnerLayer(commands, [])),
        );

        const stagedSourcePath = commands[0]?.args.at(-3);
        assert.isString(stagedSourcePath);
        const staged = PNG.sync.read(
          Buffer.from(yield* fs.readFile(stagedSourcePath as unknown as string)),
        );
        assert.deepEqual([...staged.data], [255, 255, 255, 255, 0, 0, 0, 255]);
      }),
    ),
  );
  ...
  it("normalizes explicit macOS passkey RP domains and renders required entitlements", () => {
    ...
    assert.include(entitlements, "<string>ABC1234567.app.marcode.desktop</string>");
  ...
  it("rejects incomplete macOS passkey signing configuration", () => {
    ...
    const missingProfileError = captureError({
      MARCODE_APPLE_TEAM_ID: "ABC1234567",
      MARCODE_CLERK_PASSKEY_RP_DOMAINS: "example.clerk.accounts.dev",
    });
    assert.instanceOf(missingProfileError, MissingMacPasskeyProvisioningProfileError);
    assert.equal(
      missingProfileError.message,
      "MARCODE_MACOS_PROVISIONING_PROFILE must point to an Associated Domains provisioning profile.",
    );
  ...
  it.effect("adds passkey entitlements and both renderer protocols to signed macOS builds", () =>
    Effect.gen(function* () {
      ...
      // Marcode-named fixture path; matches `provisioningProfilePath` above.
