...
// because it is the only assertion that pins the `marcode://` production deep-link
// scheme, so an upstream revert to `t3code://` fails here instead of merging silently.
it("maps capture scenes to the real application routes", () => {
  assert.equal(showcaseSceneUrl("threads", "environment-1"), "marcode://");
  assert.equal(
    showcaseSceneUrl("environments", "environment-1"),
    "marcode://settings/environments",
  );
  assert.equal(
    showcaseSceneUrl("thread", "environment-1"),
    "marcode://threads/environment-1/remote-command-center",
  );
  assert.equal(
    showcaseSceneUrl("terminal", "environment-1"),
    "marcode://threads/environment-1/remote-command-center/terminal?terminalId=term-1",
  );
  assert.equal(
    showcaseSceneUrl("review", "environment-1"),
    "marcode://threads/environment-1/remote-command-center/review",
  );
});
