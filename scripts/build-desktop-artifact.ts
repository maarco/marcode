...
export class MissingMacPasskeyDomainConfigurationError extends Schema.TaggedError<MissingMacPasskeyDomainConfigurationError>()(
  ...
  override get message(): string {
    return "MARCODE_CLERK_PUBLISHABLE_KEY or MARCODE_CLERK_PASSKEY_RP_DOMAINS is required for signed macOS passkey builds.";
  }
}
...
export class InvalidMacPasskeyPublishableKeyError extends Schema.TaggedError<InvalidMacPasskeyPublishableKeyError>()(
  ...
  override get message(): string {
    return "MARCODE_CLERK_PUBLISHABLE_KEY is invalid.";
  }
}
...
export function resolveMacPasskeySigningConfiguration(
  ...
  if (configuredRpDomains) {
    ...
    const publishableKey = env.MARCODE_CLERK_PUBLISHABLE_KEY?.trim();
    if (!publishableKey) {
...
const BuildEnvConfig = Config.all({
  ...
  wslPrebuild: Config.string("MARCODE_DESKTOP_WSL_PREBUILD").pipe(Config.option),
});
...
export const stageDesktopDmgBackground = Effect.fn("stageDesktopDmgBackground")(function* (
  ...
  // ── Marcode fork seam ── these dimensions belong to Marcode's own DMG
  // artwork in apps/desktop/resources/dmg/. Upstream retunes them with their
  // background; keep this raster size, the window bounds, and the icon
  // positions below in step with the checked-in SVG canvas (540x380).
  for (const output of [
...
const stageWslNodePtyPrebuild = Effect.fn("stageWslNodePtyPrebuild")(function* (input: {
  ...
  if (input.prebuildPath === undefined) {
    yield* Effect.logWarning(
      "[desktop-artifact] No WSL node-pty prebuild provided (--wsl-prebuild / MARCODE_DESKTOP_WSL_PREBUILD); the packaged WSL backend will not start until a Linux pty.node is bundled.",
    );
    return;
  }
...
const buildDesktopArtifactCli = Command.make("build-desktop-artifact", {
  ...
  skipBuild: Flag.boolean("skip-build").pipe(
    Flag.withDescription(
      "Skip `vp run build:desktop` and use existing dist artifacts (env: MARCODE_DESKTOP_SKIP_BUILD).",
    ),
    Flag.optional,
  ),
  ...
  wslPrebuild: Flag.string("wsl-prebuild").pipe(
    Flag.withDescription(
      "Path to a prebuilt Linux node-pty (pty.node) for the target arch, staged for the WSL backend (env: MARCODE_DESKTOP_WSL_PREBUILD).",
    ),
    Flag.optional,
  ),
}).pipe(
