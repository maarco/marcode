...
Each project uses `text.svg` for the Marcode mark and `background.svg` when the background is a vector layer. Additional layers use semantic names that describe their role and placement.

Development, nightly, and production currently use the same approved black Marcode mark on a white
rounded-square background. Release channels remain distinct through their names and build metadata,
not alternate icon artwork. Keep all three Icon Composer projects visually identical unless a new
channel-specific design is explicitly approved.
