...
const publicConfigEnvEntries = (config: RelayPublicConfig) =>
  ({
    ...
    MARCODE_MOBILE_OTLP_TRACES_URL: config.mobileTracingUrl,
    MARCODE_MOBILE_OTLP_TRACES_DATASET: config.mobileTracingDataset,
    MARCODE_MOBILE_OTLP_TRACES_TOKEN: config.mobileTracingToken,
    MARCODE_RELAY_CLIENT_OTLP_TRACES_URL: config.clientTracingUrl,
    MARCODE_RELAY_CLIENT_OTLP_TRACES_DATASET: config.clientTracingDataset,
    MARCODE_RELAY_CLIENT_OTLP_TRACES_TOKEN: config.clientTracingToken,
  }) as const;
...
export function reconcileRootEnvRelayUrl(contents: string, relayUrl: string): string {
  return reconcileRootEnvPublicConfig(contents, {
    ...
    .split("\n")
    .filter((line) => !line.startsWith("MARCODE_MOBILE_OTLP_TRACES_"))
    .filter((line) => !line.startsWith("MARCODE_RELAY_CLIENT_OTLP_TRACES_"))
    .join("\n");
}
...
const readRelayPublicConfig = Effect.fn("relay.deploy.readState")(function* (stage: string) {
  ...
  const output = yield* service.getOutput({ stack: "MarcodeRelay", stage });
