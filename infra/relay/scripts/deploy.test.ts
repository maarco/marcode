...
describe("reconcileRootEnvRelayUrl", () => {
  ...
  it("preserves unrelated root env entries while replacing a previous relay URL", () => {
    expect(
      reconcileRootEnvRelayUrl(
        "MARCODE_CLERK_PUBLISHABLE_KEY=pk_test_example\nMARCODE_RELAY_URL=https://old.example.test\n",
  ...
  it("adds the complete local client config", () => {
    expect(reconcileRootEnvPublicConfig("", config)).toBe(
      [
        ...
        "MARCODE_MOBILE_OTLP_TRACES_DATASET=t3-code-mobile-traces-dev",
        "MARCODE_MOBILE_OTLP_TRACES_TOKEN=xaat-public-ingest",
        "MARCODE_RELAY_CLIENT_OTLP_TRACES_URL=https://api.axiom.co/v1/traces",
        "MARCODE_RELAY_CLIENT_OTLP_TRACES_DATASET=t3-code-relay-client-traces-dev",
        "MARCODE_RELAY_CLIENT_OTLP_TRACES_TOKEN=xaat-relay-client-ingest",
        "",
      ].join("\n"),
    );
  });
    ...
    expect(
      reconcileRootEnvPublicConfig(
        [
          "MARCODE_CLERK_PUBLISHABLE_KEY=pk_test_example",
          "MARCODE_RELAY_URL=https://old.example.test",
          "MARCODE_MOBILE_OTLP_TRACES_URL=https://old.example.test/v1/traces",
          "MARCODE_MOBILE_OTLP_TRACES_DATASET=old-dataset",
          "MARCODE_MOBILE_OTLP_TRACES_TOKEN=old-token",
          "MARCODE_RELAY_CLIENT_OTLP_TRACES_URL=https://old.example.test/v1/traces",
          "MARCODE_RELAY_CLIENT_OTLP_TRACES_DATASET=old-client-dataset",
          "MARCODE_RELAY_CLIENT_OTLP_TRACES_TOKEN=old-client-token",
          "",
        ].join("\n"),
        config,
      ),
...
describe("serializeRelayClientTracingEnvironment", () => {
  it("serializes tracing config for downstream GITHUB_ENV loading", () => {
    expect(
      ...
      [
        "MARCODE_RELAY_CLIENT_OTLP_TRACES_URL=https://api.axiom.co/v1/traces",
        "MARCODE_RELAY_CLIENT_OTLP_TRACES_DATASET=relay",
        "MARCODE_RELAY_CLIENT_OTLP_TRACES_TOKEN=client-token",
        "",
      ].join("\n"),
    );
  });
