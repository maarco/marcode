...
@@T3_NODE_ENV_SCRIPT@@
STATE_KEY="$1"
STATE_DIR="$HOME/.marcode/ssh-launch/$STATE_KEY"
DEFAULT_SERVER_HOME="$HOME/.marcode"
...
if [ -z "$REMOTE_PORT" ]; then
  ...
  nohup env MARCODE_NO_BROWSER=1 "$RUNNER_FILE" serve --host 127.0.0.1 --port "$REMOTE_PORT" --base-dir "$DEFAULT_SERVER_HOME" >>"$LOG_FILE" 2>&1 < /dev/null &
  REMOTE_PID="$!"
  printf '%s\\n' "$REMOTE_PID" >"$PID_FILE"
  printf '%s\\n' "$REMOTE_PORT" >"$PORT_FILE"
  printf 'managed\\n' >"$MANAGED_FILE"
...
STATE_DIR="$HOME/.marcode/ssh-launch/@@MARCODE_STATE_KEY@@"
DEFAULT_SERVER_HOME="$HOME/.marcode"
RUNNER_FILE="$STATE_DIR/run-t3.sh"
mkdir -p "$STATE_DIR"
cat >"$RUNNER_FILE" <<'SH'
...
export function buildRemotePairingScript(
  ...
  return applyScriptPlaceholders(REMOTE_PAIRING_SCRIPT, {
    MARCODE_STATE_KEY: remoteStateKey(target),
    T3_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteT3RunnerScript(input)),
  });
}
