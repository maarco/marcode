...
> = {
  ...
  // Marcode's server reads MARCODE_BITBUCKET_* (see apps/server/src/sourceControl/BitbucketApi.ts);
  // upstream's copy names T3CODE_BITBUCKET_*, which would send the reader to variables nothing reads.
  bitbucket: {
    missing:
      "Bitbucket needs API credentials on the server. Set MARCODE_BITBUCKET_EMAIL and MARCODE_BITBUCKET_API_TOKEN, or MARCODE_BITBUCKET_ACCESS_TOKEN.",
    unauthenticated:
      "Bitbucket rejected the configured credentials. Check MARCODE_BITBUCKET_EMAIL and MARCODE_BITBUCKET_API_TOKEN.",
  },
};
