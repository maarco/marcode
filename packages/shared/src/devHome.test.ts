...
import { resolveGitWorktreePath, resolveWorktreeMarcodeHome } from "./devHome.ts";
...
const makeRepo = (
  ...
  Effect.acquireRelease(
    Effect.sync(() => {
      const root = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "marcode-devhome-"));
      if (kind === "worktree") {
...
describe("resolveWorktreeMarcodeHome", () => {
  it.effect("answers with .marcode before the dev runner creates it", () =>
    Effect.gen(function* () {
      const { root, nested } = yield* makeRepo("worktree");
      const home = yield* resolveWorktreeMarcodeHome(nested);
      assert.equal(home, NodePath.join(NodePath.resolve(root), ".marcode"));
