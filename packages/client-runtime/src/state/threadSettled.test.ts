...
describe("effectiveSettled", () => {
  ...
  // Marcode used to hold a terminal PR for an hour of thread idle time so a warm
  // follow-up was not buried. `pingdotgg/t3code#7454` replaced that window with
  // an event-ordering rule, so recency no longer enters into it: a terminal PR
  // that postdates the thread's last user-initiated event settles it at once,
  // and one that predates it never does.
  it("settles a terminal change request regardless of how recent the thread is", () => {
    ...
    for (const state of ["merged", "closed"] as const) {
      expect(
        effectiveSettled(recentlyActive, {
          now: NOW,
          autoSettleAfterDays: null,
          changeRequest: { state, updatedAt: NOW },
        }),
      ).toBe(true);
    }
  });

  it("does not re-settle a thread the user re-engaged after the PR closed", () => {
    const reEngaged = makeShell({ activityAt: "2026-04-09T23:30:00.000Z" });
    for (const state of ["merged", "closed"] as const) {
      expect(
        effectiveSettled(reEngaged, {
          now: NOW,
          autoSettleAfterDays: null,
          changeRequest: { state, updatedAt: "2026-04-09T23:00:00.000Z" },
        }),
      ).toBe(false);
    }
  });
