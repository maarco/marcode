...
describe("effectiveSettled", () => {
  ...
  // Marcode's warm window and upstream's settle-once anchor are independent
  // guards and BOTH must hold before a terminal PR settles a thread. Merging
  // them into one rule (or dropping either in a future upstream sync) shows up
  // here rather than as a thread that vanishes mid-conversation.
  it("requires both the warm window and the settle-once anchor to clear", () => {
    const warmActivityAt = "2026-04-09T23:30:00.000Z";
    const idleActivityAt = "2026-04-09T22:59:59.999Z";
    const mergedAfter = "2026-04-09T23:59:00.000Z";
    const mergedBefore = "2026-04-09T20:00:00.000Z";

    // Anchor clears (merge postdates the turn) but the thread is still warm.
    expect(
      effectiveSettled(makeShell({ activityAt: warmActivityAt }), {
        now: NOW,
        autoSettleAfterDays: null,
        changeRequest: { state: "merged", updatedAt: mergedAfter },
      }),
    ).toBe(false);

    // Warm window clears but the merge predates the thread's latest event.
    expect(
      effectiveSettled(makeShell({ activityAt: idleActivityAt }), {
        now: NOW,
        autoSettleAfterDays: null,
        changeRequest: { state: "merged", updatedAt: mergedBefore },
      }),
    ).toBe(false);
