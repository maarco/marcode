...
describe("effectiveSettled", () => {
  ...
  it("holds the warm window even when the merge is newer than the thread's activity", () => {
    // Upstream settles as soon as the terminal PR state is the thread's latest
    // event. Marcode's warm window still outranks that, so a PR that merges
    // mid-conversation does not bury the follow-up.
    const recentlyActive = makeShell({ activityAt: "2026-04-09T23:45:00.000Z" });
    expect(
      effectiveSettled(recentlyActive, {
        now: NOW,
        autoSettleAfterDays: null,
        changeRequest: { state: "merged", updatedAt: "2026-04-09T23:50:00.000Z" },
      }),
    ).toBe(false);
  });
