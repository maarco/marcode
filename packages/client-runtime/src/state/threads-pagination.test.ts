...
// Tests author page responses as an Option; Marcode's loader answers with a
// three-state result (found / missing / unavailable) so a deleted thread stops
// being re-fetched. Absent maps to "unavailable": the page simply did not load.
type LoaderResponse = Option.Option<OrchestrationThreadDetailSnapshot>;

const toLoadResult = (response: LoaderResponse): ThreadSnapshotLoadResult =>
  Option.match(response, {
    onNone: (): ThreadSnapshotLoadResult => ({ _tag: "unavailable" }) as const,
    onSome: (snapshot): ThreadSnapshotLoadResult => ({ _tag: "found", snapshot }) as const,
  });
