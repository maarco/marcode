 ...
 * wins in both directions; without one, a thread can auto-settle on a merged
 * PR (when autoSettleOnMerge is enabled) or always on a closed PR — both only
 * while the terminal state is the thread's latest event (see
 * changeRequestAutoSettles) and only once Marcode's idle grace has elapsed —
 * or settles on inactivity past the window.
...
export function effectiveSettled(
  ...
  if (
    ...
    // Marcode fork seam: upstream settles as soon as the terminal PR event is
    ...
    // warm follow-up conversation, so Marcode holds the row active until the
    // thread has also been idle for an hour.
