 ...
 * PR — or a merged one when `autoSettleOnMerge` is left on — but only once
 * the thread has gone idle, or on inactivity past the window. An open PR
 * blocks the inactivity path entirely. The server
...
export function effectiveSettled(
  ...
  if (changeRequestAutoSettles(options.changeRequestState, options.autoSettleOnMerge !== false)) {
    // Upstream settles on the change-request state alone. Marcode adds a warm
    // window on top: a merged/closed PR is a durable completion signal, but it
    // must not bury a thread while the user is still having a follow-up
    // conversation. Once the thread has been idle for an hour, the PR state
    // settles it.
