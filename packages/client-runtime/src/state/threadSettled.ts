 ...
 * merged PR (when `autoSettleOnMerge` is on) and always settles on a closed
 * PR — in both cases only once the thread has been idle — or settles on
 * inactivity past the window. An open PR blocks the inactivity path
 * entirely. The server
...
export function effectiveSettled(
  ...
  if (changeRequestAutoSettles(options.changeRequestState, options.autoSettleOnMerge !== false)) {
    // A settling PR state is a durable completion signal, but it must not bury
