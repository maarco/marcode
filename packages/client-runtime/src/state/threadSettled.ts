 ...
 * merged PR (subject to autoSettleOnMerge), always settles on a closed PR —
 * both only once the thread has gone idle — or settles on inactivity past
...
export function effectiveSettled(
  ...
  if (changeRequestAutoSettles(options.changeRequestState, options.autoSettleOnMerge !== false)) {
    // ── Marcode fork seam ──
    // Upstream settles immediately here. A merged/closed PR is a durable completion signal,
    // but it must not bury a thread while the user is still having a warm follow-up
    // conversation. Once the thread has been idle for an hour, the PR state settles it.
