 ...
 * wins in both directions; without one, a thread auto-settles once idle on a
 * closed PR or on a merged PR when auto-settle-on-merge is enabled, or on
...
export function effectiveSettled(
  ...
  if (changeRequestAutoSettles(options.changeRequestState, options.autoSettleOnMerge !== false)) {
    // Upstream decides *whether* the change-request state settles at all (a close
    // always does; a merge only when auto-settle-on-merge is on). Marcode then gates
    // it on idleness: the signal is durable, but it must not bury a thread while the
    // user is still having a warm follow-up conversation. Once the thread has been
    // idle for an hour, the PR state settles it. A merge with auto-settle-on-merge
    // disabled skips this entirely and can still settle via the inactivity path.
