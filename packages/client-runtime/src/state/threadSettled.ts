 ...
 * PR — and only while that terminal state is still the thread's latest event
 * (see changeRequestAutoSettles) — or on inactivity past the window. An open
...
export function effectiveSettled(
  ...
  if (
    // Upstream's settle-once rule: a terminal PR only settles the thread while
    // it still postdates every user-initiated event in it.
    changeRequestAutoSettles(options.changeRequest, {
      autoSettleOnMerge: options.autoSettleOnMerge,
      thread: shell,
    })
  ) {
    // Marcode's warm window on top of it: a merged/closed PR is a durable
    // completion signal, but it must not bury a thread while the user is
    // still having a warm follow-up conversation. Upstream's anchor only
    // looks at user-initiated events, so an agent turn that finished
    // seconds after the merge would otherwise settle the row mid-read.
