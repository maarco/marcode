...
const makeHarness = Effect.fn("TestThreadAtoms.makeHarness")(function* (options?: {
  ...
  const olderLoads = yield* Queue.unbounded<{
    ...
    readonly response: Deferred.Deferred<ThreadSnapshotLoadResult>;
    readonly closed: Deferred.Deferred<void>;
  }>();
          ...
          load: (_prepared, _threadId, window) => {
            if (window?.beforeCursor === undefined) {
              return Effect.sync(() => {
                ...
                // Marcode's loader reports a tagged result so a 404 can end the
                // subscription. Upstream's fixture speaks Option: an absent
                // snapshot here is the neutral "unavailable", not the "missing"
                // that marks the thread deleted.
                Effect.map((loaded): ThreadSnapshotLoadResult =>
                  Option.isSome(loaded)
                    ? ({ _tag: "found", snapshot: loaded.value } as const)
                    : ({ _tag: "unavailable" } as const),
                ),
              );
            }
            return Effect.gen(function* () {
              const response = yield* Deferred.make<ThreadSnapshotLoadResult>();
              const closed = yield* Deferred.make<void>();
