...
```sh
vp run --filter marcode-relay deploy -- --stage "$USER" --env-file .env.local
```
...
- `MARCODE_WEB_ROUTER_URL`: defaults to `https://app.t3.codes`.
- `MARCODE_WEB_LATEST_DOMAIN`: defaults to `latest.app.t3.codes`.
- `MARCODE_WEB_NIGHTLY_DOMAIN`: defaults to `nightly.app.t3.codes`.
...
visiting `/__marcode/channel?channel=latest` or
`/__marcode/channel?channel=nightly`; the router stores the
`marcode_web_channel` cookie and rewrites future requests on `app.t3.codes` to
...
through `/__marcode/channel` on the router domain so the user's channel cookie is
...
2. Create an explicit App ID for `com.t3tools.marcode` and enable Associated Domains.
...
- macOS build unsigned when expected signed:
  ...
  - Confirm the provisioning profile belongs to `APPLE_TEAM_ID.com.t3tools.marcode` and includes
    Associated Domains.
