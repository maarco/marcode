...
stdout/stderr at `~/.marcode/ssh-launch/<state>/server.log`.
...
- SSH-managed launch persistence: `~/.marcode/ssh-launch/<state>/server.log`
...
`<home>/userdata/logs/server.trace.ndjson` (so `~/.marcode/userdata/...` by default, or
`/custom/path/userdata/...` with `--home-dir /custom/path`), a linked worktree dev run uses
`<worktree>/.marcode/userdata/logs/server.trace.ndjson`, and an implicit dev run outside a linked
worktree uses `~/.marcode/dev/logs/server.trace.ndjson`.
...
export MARCODE_OTLP_SERVICE_NAME=t3-local
```
...
MARCODE_OTLP_SERVICE_NAME=t3-desktop \
"/Applications/Marcode (Alpha).app/Contents/MacOS/Marcode (Alpha)"
```
...
```bash
TRACE_FILE="$WORKTREE/.marcode/userdata/logs/server.trace.ndjson"
```
...
- `MARCODE_OTLP_SERVICE_NAME`: service name, default `t3-server`
