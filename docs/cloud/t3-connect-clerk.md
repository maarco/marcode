...
MARCODE_CLERK_JWT_TEMPLATE=<JWT template name>
MARCODE_CLERK_CLI_OAUTH_CLIENT_ID=<public OAuth application client ID>
...
should set `MARCODE_CLERK_PUBLISHABLE_KEY`, `MARCODE_CLERK_JWT_TEMPLATE`,
...
deployment defaults.
`vp run --filter marcode-relay deploy` invokes Alchemy from the relay directory, so Alchemy loads
...
5. Set `MARCODE_CLERK_CLI_OAUTH_CLIENT_ID` in the repository-root `.env` file and release build
...
Set `MARCODE_CLERK_JWT_TEMPLATE=t3-relay` in the repository-root `.env`, and set
...
is shared by production and non-production relay stages. The client-facing `MARCODE_RELAY_URL` still
...
Local desktop development uses `marcode-dev://app`, while packaged builds use `marcode://app`. Add the
matching origin to each Clerk instance's Backend API `allowed_origins` array as well. The development
Clerk instance should only need `marcode-dev://app`; the production Clerk instance should only need
`marcode://app`. `@clerk/electron` owns the native request adapter, encrypted Clerk token persistence,
...
The production macOS bundle ID is `com.t3tools.marcode`. To enable native passkeys:

1. Create an explicit macOS App ID for `com.t3tools.marcode` in the Apple Developer portal and enable
   **Associated Domains**.
...
4. Confirm Clerk serves `https://<frontend-api>/.well-known/apple-app-site-association` and that
   `webcredentials.apps` contains `<TEAM_ID>.com.t3tools.marcode`.
...
MARCODE_MACOS_PROVISIONING_PROFILE=/absolute/path/to/marcode.provisionprofile
# Optional: comma-separated override when Clerk's RP ID differs from the Frontend API hostname.
MARCODE_CLERK_PASSKEY_RP_DOMAINS=example.clerk.accounts.dev,clerk.example.com
```

When `MARCODE_CLERK_PASSKEY_RP_DOMAINS` is absent, the build derives the RP domain from
`MARCODE_CLERK_PUBLISHABLE_KEY`. Signed macOS builds fail early if the Team ID, provisioning profile,
...
installed app executable with `VITE_DEV_SERVER_URL` and `MARCODE_PORT` set. Rebuild the signed app
