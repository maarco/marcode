...
Marcode runs Google's official Antigravity ACP agent on your selected environment.
...
**Sign in with Google** and complete the browser sign-in. Wait for Marcode to confirm
...
hostname. Only that Marcode sign-in session can finish the attempt. If it expires,
...
Use Antigravity's native `/plan` command for planning. Marcode's separate Plan mode
...
Marcode keeps conversation history and file diffs, but Antigravity cannot rewind
...
Put project skills in `.agents/skills`. Marcode also reads `.gemini/skills` and the
