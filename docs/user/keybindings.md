...
The same configuration lives in `~/.marcode/userdata/keybindings.json` on the machine running the
server, if you prefer editing it directly. Marcode writes the built-in defaults into that file on
