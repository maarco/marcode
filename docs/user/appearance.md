...
On web and desktop, choose **Create theme** to adjust a palette, or import a Marcode or VS Code
...
takes precedence. If the server stops publishing the selected theme, Marcode falls back to its
standard theme.
...
Save a theme exported from Marcode into `~/.t3/userdata/themes/` on the server, or the `themes`
...
Set `appearance` to `light` or `dark` and supply hex colors for `canvas` and `accent`. Marcode
