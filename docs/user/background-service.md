...
**Linux** uses a systemd user unit at `~/.config/systemd/user/marcode.service`. The service starts
when the machine boots and keeps running after you log out (lingering is enabled during install).

**macOS** uses a launch agent at `~/Library/LaunchAgents/com.t3tools.marcode.service.plist`. It
