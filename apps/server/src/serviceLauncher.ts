...
/**
 * The unit rendered by `renderBootServiceUnit` sets `MARCODE_HOME`, the same
 * base-dir variable the Marcode server itself reads. Upstream names this
 * `T3CODE_HOME`; the launcher must follow Marcode's unit or it exits before it
 * can start a server.
 */
export function resolveLauncherBaseDir(env: NodeJS.ProcessEnv): string {
  const baseDir = env.MARCODE_HOME?.trim();
  if (baseDir === undefined || baseDir === "") {
    throw new Error("MARCODE_HOME is required by the Marcode service launcher.");
  }
  return baseDir;
}
