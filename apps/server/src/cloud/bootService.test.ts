...
it("quotes systemd values containing spaces and escapes percent specifiers", () => {
  ...
  const unit = BootService.renderBootServiceUnit({
    ...
    unitPath: "/home/me/.config/systemd/user/marcode.service",
  });
  assert.include(unit, 'ExecStart="/home/me/my tools/node" "/home/me/T3 Data/bin.mjs" serve');
  assert.include(unit, 'Environment=MARCODE_HOME="/home/me/T3 Data"');
...
it.layer(NodeServices.layer)("BootService", (it) => {
    ...
    Effect.gen(function* () {
      ...
      assert.deepEqual(
        ...
        [
          ...
          "systemctl --user enable marcode.service",
      ...
      const unitPath = path.join(dirs.home, ".config", "systemd", "user", "marcode.service");
      const unit = yield* fs.readFileString(unitPath);
      assert.include(unit, `ExecStart=/usr/local/bin/node ${dirs.stableEntry} serve`);
      assert.include(unit, `Environment=MARCODE_HOME=${dirs.baseDir}`);
  ...
  it.effect("reports an installed-but-stale unit so the lifecycle can offer a repair", () =>
    Effect.gen(function* () {
      ...
      yield* fs.writeFileString(
        path.join(unitDir, "marcode.service"),
  ...
  it.effect("fails on non-Linux platforms without touching the filesystem", () =>
    Effect.gen(function* () {
      ...
      assert.isFalse(
        yield* fs.exists(path.join(dirs.home, ".config", "systemd", "user", "marcode.service")),
      );
  ...
  it.effect("removes the unit file when an activation step fails", () =>
    Effect.gen(function* () {
      ...
      assert.isTrue(
        commands.some(
          ({ command, args }) =>
            command === "systemctl" && args.join(" ") === "--user disable --now marcode.service",
        ),
      );
    }),
  );
  ...
  it.effect("restores the previous unit when a repair cannot activate", () =>
    Effect.gen(function* () {
      ...
      assert.isTrue(
        repairCommands.some(
          ({ command, args }) =>
            command === "systemctl" && args.join(" ") === "--user restart marcode.service",
        ),
      );
    }),
  );
