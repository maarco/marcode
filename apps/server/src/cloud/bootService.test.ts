...
// Marcode fork boundary: upstream's launch agent exports T3CODE_HOME and is
// labelled com.t3tools.t3code.service. resolveLauncherBaseDir only accepts
// MARCODE_HOME, and a shared label would make a Marcode install and an
// upstream T3 Code install fight over one LaunchAgent plist — the same reason
// BOOT_SERVICE_UNIT_FILE is marcode.service. Both are pinned here so an
// upstream sync that rewrites this renderer fails loudly.
it("exports the Marcode home variable and label the launcher requires", () => {
  const plist = BootService.renderBootServicePlist(macPlan, { homeDir: "/Users/theo" });

  expect(plist).toContain("<key>MARCODE_HOME</key>\n    <string>/Users/theo/.t3</string>");
  expect(plist).not.toContain("T3CODE_HOME"); // marcode-home-guard:allow
  ...
  // The compatibility identifier upstream and Marcode share stays upstream-shaped.
  expect(BootService.BOOT_SERVICE_UNIT_ENV).toBe("T3_BOOT_SERVICE_UNIT");
  expect(plist).toContain(
    `<key>${BootService.BOOT_SERVICE_UNIT_ENV}</key>\n    <string>${BootService.BOOT_SERVICE_PLIST_FILE}</string>`,
  );
});
...
it.layer(NodeServices.layer)("boot service install", (it) => {
  ...
  it.effect("installs, reports current state, and uninstalls on macOS", () =>
    Effect.gen(function* () {
      ...
      expect(
        timeouts.get(`launchctl bootout --wait gui/501/${BootService.BOOT_SERVICE_LAUNCHD_LABEL}`),
      ).toEqual(Duration.seconds(120));
    }),
  );
  ...
  it.effect("ignores a bootout for an agent that is not loaded", () =>
    Effect.gen(function* () {
      ...
      yield* service.install;
      control.failCommand = `launchctl bootout --wait gui/501/${BootService.BOOT_SERVICE_LAUNCHD_LABEL}`;
