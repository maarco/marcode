...
const macPlan = {
  ...
  unitPath: "/Users/theo/Library/LaunchAgents/app.marcode.service.plist",
};

// The launch agent's label is the macOS half of the coexistence rule the
// systemd unit test above pins: an upstream `com.t3tools.t3code.service` label
// would make a Marcode install and an upstream install the same launchd job.
it("labels the launch agent with Marcode's own service identity", () => {
  expect(BootService.BOOT_SERVICE_LAUNCHD_LABEL).toBe("app.marcode.service");
  expect(BootService.BOOT_SERVICE_PLIST_FILE).toBe("app.marcode.service.plist");
});
...
// The macOS launch agent arrived from upstream in a clean merge, exporting the
// upstream `T3CODE_HOME`. Marcode's launcher requires `MARCODE_HOME` and exits
// without it, so the service would install and never come up. Pin both
// renderers to the same key so the next upstream rewrite fails here.
it("hands the launch agent the base dir under the key the launcher reads", () => {
  const plist = BootService.renderBootServicePlist(macPlan, { homeDir: "/Users/theo" });

  expect(plist).toContain("<key>MARCODE_HOME</key>\n    <string>/Users/theo/.t3</string>");
  expect(plist).not.toContain("T3CODE_HOME");
  // The unit env var *name* stays upstream-shaped: a compatibility identifier
  // read by installed units, not user-visible branding.
  expect(plist).toContain(`<key>${BootService.BOOT_SERVICE_UNIT_ENV}</key>`);
});
...
it.layer(NodeServices.layer)("boot service install", (it) => {
  it.effect("installs, reports current state, and uninstalls", () =>
    Effect.gen(function* () {
      ...
      expect(timeouts.get("systemctl --user disable --now marcode.service")).toEqual(
  ...
  it.effect("installs, reports current state, and uninstalls on macOS", () =>
    Effect.gen(function* () {
      ...
      expect(plan.unitPath.endsWith("Library/LaunchAgents/app.marcode.service.plist")).toBe(true);
      expect(parseServiceState(yield* fs.readFileString(statePath))).toEqual({
        protocol: SERVICE_LAUNCHER_PROTOCOL,
        activeVersion: "1.2.3",
      });
      ...
      expect(timeouts.get("launchctl bootout --wait gui/501/app.marcode.service")).toEqual(
        Duration.seconds(120),
      );
    }),
  );
  ...
  it.effect("restarts the launch agent when repair fails", () =>
    Effect.gen(function* () {
      ...
      expect(commands.filter((command) => command.startsWith("launchctl "))).toEqual([
        "launchctl bootout --wait gui/501/app.marcode.service",
        "launchctl enable gui/501/app.marcode.service",
  ...
  it.effect("ignores a bootout for an agent that is not loaded", () =>
    Effect.gen(function* () {
      ...
      yield* service.install;
      control.failCommand = "launchctl bootout --wait gui/501/app.marcode.service";
