...
it("exports Marcode's base-dir variable to the launch agent", () => {
  // Upstream's plist exports T3CODE_HOME. `resolveLauncherBaseDir` reads
  // MARCODE_HOME and throws without it, so an upstream sync that reinstates the
  // upstream name must fail here instead of shipping a service that never boots.
  const plist = BootService.renderBootServicePlist(macPlan, macRenderOptions);
  ...
  const runner = ProcessRunner.ProcessRunner.of({
    run: Effect.fn("test.run_boot_service_command")(function* (
      ...
      if (!failed && command === "loginctl enable-linger --no-ask-password 501")
        control.linger = "yes";
      if (!failed && command === "systemctl --user enable marcode.service") control.enabled = true;
      if (!failed && command === "systemctl --user restart marcode.service") control.active = true;
      if (
        control.stateAfterStop !== undefined &&
        (command === "systemctl --user stop marcode.service" ||
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(commands).not.toContain("systemctl --user stop marcode.service");
      }),
  );
    ...
    (platform) =>
      Effect.gen(function* () {
        ...
        expect(
          ...
          platform === "linux"
            ? ["systemctl --user stop marcode.service", "systemctl --user restart marcode.service"]
            : [
