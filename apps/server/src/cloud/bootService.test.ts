...
// The launch agent inherits nothing from the user's shell, so the plist is the
// only place MARCODE_HOME can come from. Upstream's plist exports T3CODE_HOME,
// which resolveLauncherBaseDir rejects — the agent would exit before starting a
// server. Pinned here so an upstream rewrite of the renderer fails loudly.
it("exports the base directory under the name Marcode's launcher reads", () => {
  const plist = BootService.renderBootServicePlist(macPlan, { homeDir: "/Users/theo" });
