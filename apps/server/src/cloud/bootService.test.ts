...
// Marcode fork seam: the launcher requires MARCODE_HOME, so the plist must
// export the Marcode-shaped env var. A future upstream rename to T3CODE_HOME
// would leave the launcher unable to boot and this test failing loudly.
it("declares MARCODE_HOME for Marcode's service launcher", () => {
  const plist = BootService.renderBootServicePlist(macPlan, { homeDir: "/Users/theo" });

  expect(plist).toContain("<key>MARCODE_HOME</key>");
  expect(plist).not.toContain("<key>T3CODE_HOME</key>");
});
