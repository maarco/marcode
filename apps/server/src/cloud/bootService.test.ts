...
const macPlan = {
  ...
  unitPath: "/Users/theo/Library/LaunchAgents/com.t3tools.marcode.service.plist",
};
...
it("names Marcode's base-dir variable in the launch agent", () => {
  const plist = BootService.renderBootServicePlist(macPlan, { homeDir: "/Users/theo" });

  // Upstream's plist exports T3CODE_HOME. `resolveLauncherBaseDir` reads
  // MARCODE_HOME and throws without it, so an upstream-shaped key here means the
  // macOS background service never starts. Pin it so a sync conflicts loudly.
  expect(plist).toContain("<key>MARCODE_HOME</key>");
  expect(plist).not.toContain("T3CODE_HOME");
  expect(plist).toContain("<string>/Users/theo/.t3</string>");
});

it("ships a Marcode-specific launch-agent label so an upstream install can coexist", () => {
  expect(BootService.BOOT_SERVICE_LAUNCHD_LABEL).toBe("com.t3tools.marcode.service");
  expect(BootService.BOOT_SERVICE_PLIST_FILE).toBe("com.t3tools.marcode.service.plist");
});
...
it.layer(NodeServices.layer)("boot service install", (it) => {
  ...
  it.effect("installs, reports current state, and uninstalls on macOS", () =>
    Effect.gen(function* () {
      ...
      expect(plan.unitPath.endsWith("Library/LaunchAgents/com.t3tools.marcode.service.plist")).toBe(
        true,
      );
      expect(parseServiceState(yield* fs.readFileString(statePath))).toEqual({
        protocol: SERVICE_LAUNCHER_PROTOCOL,
      ...
      expect(timeouts.get("launchctl bootout --wait gui/501/com.t3tools.marcode.service")).toEqual(
  ...
  it.effect("restarts the launch agent when repair fails", () =>
    Effect.gen(function* () {
      ...
      expect(commands.filter((command) => command.startsWith("launchctl "))).toEqual([
        "launchctl bootout --wait gui/501/com.t3tools.marcode.service",
        "launchctl enable gui/501/com.t3tools.marcode.service",
  ...
  it.effect("ignores a bootout for an agent that is not loaded", () =>
    Effect.gen(function* () {
      ...
      yield* service.install;
      control.failCommand = "launchctl bootout --wait gui/501/com.t3tools.marcode.service";
