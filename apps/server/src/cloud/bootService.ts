...
export function renderBootServicePlist(
  ...
  return [
    ...
    // ── Marcode fork seam ──
    // Upstream's plist exports T3CODE_HOME. The service launcher
    // (`resolveLauncherBaseDir`) reads MARCODE_HOME and exits without it, so
    // the launchd unit must match the systemd unit above.
