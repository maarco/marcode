...
// Marcode-namespaced for the same reason BOOT_SERVICE_NAME is: upstream's
// com.t3tools.t3code.service would make a Marcode install and an upstream
// T3 Code install fight over one LaunchAgent plist. The `.service` suffix
// keeps the label distinct from the desktop app's bundle id
// (app.marcode.desktop), so launchd and TCC records never collide.
...
export function renderBootServicePlist(
  ...
  return [
    ...
    `  <dict>`,
    // MARCODE_HOME, not upstream's T3CODE_HOME: resolveLauncherBaseDir requires
    // the Marcode name and exits before it can start the server without it.
