...
export function renderBootServicePlist(
  ...
  return [
    ...
    `  <dict>`,
    // Marcode fork seam: the launcher reads MARCODE_HOME, not upstream's
    // T3CODE_HOME. A plist that exports the upstream name makes the launch
    // agent exit before it can start a server. Mirrors the systemd unit above.
