...
// Marcode fork seam: upstream labels the launch agent `com.t3tools.t3code`.
// The same coexistence rule as the systemd unit above applies on macOS — a
// Marcode agent and an upstream T3 Code agent must be separate launchd jobs —
// so the label follows Marcode's desktop bundle id (app.marcode.desktop). The
// `.service` suffix keeps it distinct from that bundle id, so launchd and TCC
// records never collide.
export const BOOT_SERVICE_LAUNCHD_LABEL = "app.marcode.service";
...
export function renderBootServicePlist(
  ...
  return [
    ...
    `  <dict>`,
    // Marcode fork seam: the launcher requires MARCODE_HOME and exits without
    // it, so both unit renderers export the same key (see serviceLauncher.ts).
