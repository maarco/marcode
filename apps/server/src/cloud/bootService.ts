...
// Marcode ships its own launch-agent label for the same reason it ships its own
// systemd unit name: an upstream T3 Code install writes
// `com.t3tools.t3code.service.plist` and the two must not overwrite each other.
// The `.service` suffix keeps the label distinct from the desktop app's bundle
// id (com.t3tools.marcode), so launchd and TCC records never collide.
export const BOOT_SERVICE_LAUNCHD_LABEL = `com.t3tools.${BOOT_SERVICE_NAME}.service`;
...
export function renderBootServicePlist(
  ...
  return [
    ...
    `  <dict>`,
    // Upstream writes `T3CODE_HOME` here. The Marcode launcher reads
    // `MARCODE_HOME` (see `resolveLauncherBaseDir`) and exits without it, so the
    // launch agent must name Marcode's variable.
...
export const make = Effect.fn("cloud.boot_service.make")(function* (input: {
  ...
  const status: BootService["Service"]["status"] = Effect.gen(function* () {
    ...
    // A unit is current only when every artifact install would write now is
    // present and at this CLI version; any mismatch makes connect offer a
    // repair (a pinned runtime under ~/.marcode can be deleted to reclaim space).
