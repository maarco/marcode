...
describe("CheckpointReactor", () => {
  ...
  it("does not adopt a temporary placeholder checkout as the thread branch", async () => {
    const harness = await createHarness({
      seedFilesystemCheckpoints: false,
      threadBranch: "t3code/original-branch",
      // Placeholder-shaped means `<WORKTREE_BRANCH_PREFIX>/<8 hex>`, and Marcode
      // renamed that prefix. Deriving it keeps this fixture actually temporary
      // instead of quietly becoming an ordinary branch the reactor should adopt.
      localStatusRefName: `${WORKTREE_BRANCH_PREFIX}/0a1b2c3d`,
    });
