...
// ── Marcode fork seam ──
// Deliberately upstream's manifest, not the fork's. Model classification is
// upstream-owned data with no Marcode divergence, and pointing at
// maarco/marcode's main would only refresh on a sync — losing the whole point
// of hosting it. Same precedent as the triage playbook URL in
// `apps/server/src/cli/triagePrompt.ts`. Revisit only if Marcode ever ships a
// provider model upstream does not have.
