...
// Upstream's fixtures hardcode `.t3`, its default home directory. Marcode's
// `resolveBaseDir` defaults to `~/.marcode`, so a fixture pinned to upstream's
// name makes the fake desktop listen on a socket the command never dials.
const DEFAULT_HOME_DIRECTORY_NAME = ".marcode";
...
describe("t3 app", () => {
  ...
  // Upstream reads T3CODE_HOME for this command. Marcode's base-dir variable is
  // MARCODE_HOME everywhere else in the CLI, so a sync that reinstates upstream's
  // name must fail here rather than silently target the default install.
  it.effect("ignores upstream's T3CODE_HOME and reads MARCODE_HOME", () =>
    withTempDirectory("t3-app-home-env-test-", (root) =>
      Effect.gen(function* () {
        const baseDir = NodePath.join(root, "marcode-home");
        const workingDirectory = yield* HostProcessWorkingDirectory;
        const desktop = yield* fakeDesktop({ baseDir });

        yield* runCli(["app"], { MARCODE_HOME: baseDir, T3CODE_HOME: NodePath.join(root, "t3") });
  ...
  it.effect("prefers the installed desktop app when a dev desktop is also running", () =>
    withTempDirectory("t3-app-preferred-test-", (root) =>
      Effect.gen(function* () {
        ...
        const baseDir = NodePath.join(root, DEFAULT_HOME_DIRECTORY_NAME);
        const desktop = yield* fakeDesktop({ baseDir });
