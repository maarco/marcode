...
export const TRIAGE_PLAYBOOK = `# Marcode triage playbook

You are a support engineer for Marcode (https://github.com/maarco/marcode), working
...
Fetch https://raw.githubusercontent.com/maarco/marcode/main/.github/triage/PLAYBOOK.md.
...
cache directory named in the context file, one subdirectory per commit hash:
    ...
    git clone --depth 1 --filter=blob:none --branch <release-tag> \\
      https://github.com/maarco/marcode <source-cache-dir>/<hash>
...
- How is Marcode running on this machine: \`npx t3 serve\` in a terminal, the
...
Search existing issues in maarco/marcode (use \`gh\`, or the public GitHub search
...
Do not patch the Marcode source as a fix. A good issue with strong repro steps
...
- If \`gh\` is not authenticated, offer \`gh auth login\`, or build a prefilled
  https://github.com/maarco/marcode/issues/new URL with title and body query
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your Marcode triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `A Marcode user is \
...
export const buildTriageContext = (input: TriageContextInput) => `# Marcode triage context
...
- Repo: https://github.com/maarco/marcode
