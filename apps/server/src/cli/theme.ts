...
// ── Marcode fork seam ── upstream reads T3CODE_HOME here. Marcode's other CLI
// commands (`cli/config.ts`, `cli/pair.ts`, `cli/triage.ts`) read MARCODE_HOME,
// so reading upstream's name would make this the one command that ignores the
// user's exported home and silently targets the default install.
