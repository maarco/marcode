...
// ── Marcode fork seam ──
// Upstream reads T3CODE_HOME here; Marcode's base-dir variable is MARCODE_HOME
// everywhere else in this CLI (`config.ts`, `pair.ts`, `triage.ts`). Reading the
// upstream name would make `t3 theme` the one command that ignores the user's
// configured home and silently writes to the default install.
