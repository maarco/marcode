...
describe("t3 theme", () => {
  ...
  it.effect("honors MARCODE_HOME like the rest of the CLI", () =>
    Effect.gen(function* () {
      const baseDir = makeBaseDir();
      yield* runCli(["theme", "set", "ocean"]).pipe(
        Effect.provide(
          ConfigProvider.layer(ConfigProvider.fromEnv({ env: { MARCODE_HOME: baseDir } })),
        ),
      );
      assert.equal(readSettings(baseDir).defaultTheme, "ocean");
    }),
  ...
  // Upstream reads T3CODE_HOME here. If a sync reinstates it, this command
  // would follow upstream's variable instead of the user's Marcode home, so
  // fail loudly instead. Both variables are set so the assertion never falls
  // through to the real default install.
  it.effect("prefers MARCODE_HOME over upstream's T3CODE_HOME", () =>
    Effect.gen(function* () {
      const marcodeHome = makeBaseDir();
      const upstreamHome = makeBaseDir();
      yield* runCli(["theme", "set", "ocean"]).pipe(
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: { MARCODE_HOME: marcodeHome, T3CODE_HOME: upstreamHome },
            }),
          ),
        ),
      );
      assert.equal(readSettings(marcodeHome).defaultTheme, "ocean");
