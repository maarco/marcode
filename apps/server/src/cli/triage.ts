...
export const triageCommand = Command.make("triage", {
  ...
  Command.withDescription(
    "Investigate a Marcode problem on this machine with claude or codex, and help file a good issue.",
  ),
  Command.withHandler((flags) =>
    Effect.gen(function* () {
      ...
      // --base-dir wins; MARCODE_HOME is its documented env equivalent (same
      // precedence as `t3 pair`). Upstream reads T3CODE_HOME here; Marcode's
      // home variable is MARCODE_HOME, so reading upstream's name would send
      // triage at the default base dir and collect the wrong machine's state.
