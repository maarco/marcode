...
const appEnvironment = Config.all({
  // ── Marcode fork seam ── upstream reads T3CODE_HOME here. Every other Marcode
  // CLI command (`cli/config.ts`, `cli/pair.ts`, `cli/theme.ts`, `cli/triage.ts`)
  // reads MARCODE_HOME, so keeping upstream's name would make `t3 app` the one
  // command that ignores an exported home and targets the default install.
...
const runAppCommand = Effect.fn("cli.app")(function* (flags: {
  ...
  const configuredBaseDir = Option.getOrUndefined(flags.baseDir) ?? environment.marcodeHome;
  const baseDir = yield* resolveBaseDir(configuredBaseDir);
  const allowDevFallback = Option.isNone(flags.baseDir) && !environment.marcodeHome?.trim();
