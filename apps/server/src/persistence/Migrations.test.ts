...
it("keeps Marcode migration 33 and appends upstream thread lifecycle migrations", () => {
  ...
  // 033, so every shared migration sits one id higher here. Widen the slice
  // when a sync appends, so id 33 stays the anchor this pin is built around.
  assert.deepStrictEqual(
    migrationEntries.slice(-12).map(([id, name]) => [id, name]),
    [
      ...
      // Upstream's 042 and 043, renumbered on the way in by t3code@2daff8c2.
