...
it("keeps Marcode migration 33 and appends upstream thread lifecycle migrations", () => {
  ...
  //
  // Anchored at 33 rather than a fixed tail length: a `slice(-n)` window walks
  // off the start as upstream appends, and this assertion would quietly stop
  // covering the id it is named for.
  const marcodeOwnedIndex = migrationEntries.findIndex(([id]) => id === 33);
  assert.notEqual(marcodeOwnedIndex, -1);
  assert.deepStrictEqual(
    ...
    [
      ...
      // Upstream's 042 and 043, renumbered on the way in.
