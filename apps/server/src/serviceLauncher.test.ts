...
// The systemd unit Marcode installs exports MARCODE_HOME, not upstream's
// T3CODE_HOME. Reading the wrong variable makes the launcher exit before it can
// start a server, so pin the pairing here.
it("reads the base dir from the variable Marcode's systemd unit exports", () => {
  assert.equal(
    resolveLauncherBaseDir({ MARCODE_HOME: " /home/theo/.marcode " }),
    "/home/theo/.marcode",
  );
