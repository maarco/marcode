...
function isT3ManagedWorktree(
  ...
  return (
    ...
    // Marcode fork seam: upstream's fallback matches their `/.t3/worktrees/`
    // home. Marcode's home directory is `.marcode` (packages/shared/devHome.ts),
    // so a worktree outside the configured `worktreesDir` — a different home, a
    // linked git worktree's own `.marcode` — is only recognised under this name.
    normalized.includes("/.marcode/worktrees/")
  );
}
