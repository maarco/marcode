  ...
  describe("scan", () => {
    ...
    it.effect("excludes T3-managed worktree sandboxes", () =>
      Effect.gen(function* () {
        ...
        const worktreeCwd = path.join(claudeHomePath, ".marcode", "worktrees", "marcode", "wt-1");
    ...
    it.effect(
      "excludes sandboxes under the configured worktrees dir without .marcode in the path",
      () =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const claudeHomePath = yield* makeTempDir("t3code-claude-home-");
          ...
          // dir contains no `.marcode` segment — only the config-based prefix match
