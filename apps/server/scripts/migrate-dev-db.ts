 ...
 * ~/.marcode database, then run this checkout's migrations against it.
 *
 * `vp run migrate-dev-db` from a worktree:
 *   1. Nukes `<worktree>/.marcode/userdata/state.sqlite`.
...
import { resolveWorktreeMarcodeHome } from "@t3tools/shared/devHome";
...
export class MigrateDevDbNotInWorktreeError extends Schema.TaggedErrorClass<MigrateDevDbNotInWorktreeError>()(
  ...
  override get message(): string {
    return "Not inside a linked git worktree. Pass --base-dir to target an isolated .marcode directory.";
  }
}
...
export class MigrateDevDbSharedHomeError extends Schema.TaggedErrorClass<MigrateDevDbSharedHomeError>()(
  ...
  override get message(): string {
    return "Refusing to rebuild the shared ~/.marcode database. Use an isolated --base-dir.";
  }
}
...
export interface RunMigrateDevDbInput {
  /** Isolated .marcode directory. Defaults to `<worktree>/.marcode` of the cwd. */
  readonly baseDir?: string | undefined;
  /** Source database. Defaults to `~/.marcode/userdata/state.sqlite`. */
...
export const runMigrateDevDb = Effect.fn("runMigrateDevDb")(function* (
  ...
  const sharedHome = path.resolve(options.sharedHome ?? path.join(NodeOS.homedir(), ".marcode"));
  ...
  const baseDir =
    input.baseDir !== undefined
      ...
      : yield* resolveWorktreeMarcodeHome(process.cwd());
...
export const migrateDevDbCommand = Command.make(
  "migrate-dev-db",
  {
    ...
    baseDir: Flag.string("base-dir").pipe(
      ...
      Flag.withDescription(
        "Isolated .marcode directory. Defaults to the current worktree's .marcode.",
      ),
    ),
    source: Flag.string("source").pipe(
      Flag.optional,
      Flag.withDescription("Source database. Defaults to ~/.marcode/userdata/state.sqlite."),
    ),
  },
  ...
  Command.withDescription(
    "Rebuild the worktree dev database from a pruned snapshot of the real ~/.marcode data, then run migrations.",
  ),
);
