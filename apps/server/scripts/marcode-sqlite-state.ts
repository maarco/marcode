...
export class SqliteStateSharedHomeMutationError extends Schema.TaggedError<SqliteStateSharedHomeMutationError>()(
  ...
  override get message(): string {
    return "Refusing to mutate the shared ~/.marcode database. Use an isolated --base-dir.";
  }
}
...
// ── Marcode fork seam ── upstream's t3-sqlite-state.ts is marcode-sqlite-state.ts
// here; the command name follows the file so `node <path>` and --help agree.
const t3SqliteStateCommand = Command.make(
  "marcode-sqlite-state",
  {
