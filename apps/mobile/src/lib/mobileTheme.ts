...
export const MOBILE_THEME_OPTIONS: ReadonlyArray<{
  ...
  // The id stays `t3-code`: it is a persisted client setting, so renaming it
  // would reset every device to the default palette on upgrade. Only the
  // picker label carries Marcode's product identity.
  { id: DEFAULT_MOBILE_THEME_ID, label: "Marcode" },
