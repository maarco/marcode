...
describe("buildUnifiedWorkspaceTree — leaf capability flags", () => {
  ...
  // hostname of every URL shortcut it accepts leaves the machine. Upstream's
  // isPublicFaviconHost guard makes it refuse private, loopback and reserved
  // hosts; a shortcut pointing at a dev server or an intranet host must render
  // its fallback glyph instead. Asserted because the leak is invisible in the
  // UI, which shows a globe either way.
