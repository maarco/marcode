...
// ── Marcode fork seam ── the kind list lives in marcodeRightPanelPolicy.ts.
// Marcode drops upstream's file/files kinds (the floating Code editor is the
// only file-editing surface here) and keeps every other upstream surface.
// Keep this export stable for client callers.
...
interface RightPanelStoreState {
  ...
  // "file" is not a Marcode surface kind, so it needs no exclusion here.
...
interface LegacyThreadState {
  ...
  // Persisted by upstream's device surface; unknown-shaped until validated below.
  dismissedDeviceSurfaceIds?: unknown;
}
