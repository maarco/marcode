...
// ── Marcode fork seam ──
// Opens the diff's *working* file in the floating editor, where upstream opens it in the
// right panel's file surface. This is deliberately not `openDiffFile` (editor-store.ts):
// that renders a diff view, a different feature.
//
// `filePath` comes from `resolveFileDiffPath` (lib/diffRendering.ts), which reads
// `FileDiffMetadata.name`/`prevName` — a patch-header path from `@pierre/diffs`. Unlike
// terminal-link text, a unified-diff filename has no `:line:col` suffix convention
// (position lives in hunk headers), so it cannot collide with `resolvePathLinkTarget`'s
// line-suffix encoding and needs no splitting before reaching the bridge.
