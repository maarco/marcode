...
export function openDiffFilePrimaryAction({
  ...
  // ── Marcode fork seam ──
  // Opens the diff's *working* file in the floating editor — Marcode's only
  // file-editing surface — where upstream opens a right-panel `file` surface.
  // This is deliberately not `openDiffFile` (editor-store.ts): that renders a
  // diff view, a different feature.
  //
