  ...
  return useCallback(
    (
      ...
      return (async () => {
        ...
        if (modelSelectionOverride) {
          // After sticky state so the resolved selection (project default or
          // the viewed thread's exact selection, options included) wins over
          // the globally sticky one. replaceOptions: the override is a
