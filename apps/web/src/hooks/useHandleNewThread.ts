  ...
  return useCallback(
    (
      ...
      return (async () => {
        ...
        // After sticky state so the viewed thread's exact selection (model plus
        // options like effort and context window) wins over the globally sticky
        // one. replaceOptions: the override is a complete snapshot — absent
        // options mean "no options", not "keep whatever sticky state just wrote".
