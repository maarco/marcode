...
describe("client telemetry metadata", () => {
  ...
  it("uses Electron's client platform for desktop", () => {
    expect(
      ...
      // Marcode labels every client with its own product name; upstream reports
      // "T3 Code Desktop"/"T3 Code Web" here. Keep this pinned so a sync that
      // reinstates the upstream label fails instead of shipping it.
