...
describe("rightPanelStore", () => {
  ...
  it.each(["one", "all", "others", "right"])(
    ...
    (mode) => {
      ...
      // Upstream pairs the device tab with their "files" surface; Marcode
      // retired that surface, so "diff" stands in as the companion singleton.
      ...
      if (mode === "others") store.closeOtherSurfaces(refA, "diff");
      if (mode === "right") store.closeSurfacesToRight(refA, "diff");
  ...
  it("keeps manual choices scoped to their thread and environment", () => {
    ...
    const store = useRightPanelStore.getState();
    const revision = store.getUserActionRevision(refA);
    store.openBrowser(refB, "tab-a");
    store.openBrowser(otherEnvironment, "tab-a");

    expect(store.openProactive(refA, completedDiff, revision)).toBe(true);
    expect(selectActiveRightPanel(useRightPanelStore.getState().byThreadKey, refB)).toBe("preview");
    expect(
