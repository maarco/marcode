...
export function useCloudLinkController() {
  ...
  const reportUpdateFailure = (cause: unknown) => {
    const message =
      cause instanceof Error ? cause.message : "Could not update Marcode Connect access.";
    ...
    toastManager.add({
      type: "error",
      title: "Could not update Marcode Connect",
      description: message,
      data: traceId
        ? {
  ...
  const reconcileCloudState = async (desired: CloudLinkDesiredState): Promise<boolean> => {
    ...
    if (!wantsLink) {
      ...
      if (!clerkToken) {
        reportUpdateFailure(new Error("Sign in to Marcode Connect before enabling this."));
        return false;
      }
