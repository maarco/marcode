...
const MarkdownFileLink = memo(function MarkdownFileLink({
  ...
  const handleOpenInFilePreview = useCallback(() => {
    ...
    // Routed through the caller so a bare filename reference goes through the
    // workspace index lookup before the floating editor opens it.
...
function ChatMarkdown({
  ...
  //
  // Marcode fork seam: upstream opens the resolved path in the right panel's
  // `file` surface. Marcode retired that surface — the floating Code editor is
  // the only file-editing surface — so only the destination changes here. The
  // index lookup above it is upstream's fix and must survive every sync.
