...
interface MarkdownFileLinkProps {
  ...
  iconPath: string;
  displayPath: string;
  /** What the file surface opens: workspace-relative inside the workspace, the
      absolute host path outside it, null when it cannot show the file. */
...
export const ChatMarkdownAssetImage = memo(function ChatMarkdownAssetImage(props: {
  ...
  const actionsSource: MediaActionSource = {
    ...
    ...(reference ? { reference } : {}),
    // ── Marcode fork seam ── upstream opens this through the right-panel
    // `file` surface; Marcode's only file-editing surface is the floating
    // Code editor, so the media action lands there instead.
      ...
      ? {
          onOpenFile: () =>
            openFileInFloatingEditor({
              ...
              relativePath,
              assetThreadId: resource.threadId,
            }),
        }
      : {}),
  };
...
const MarkdownFileLink = memo(function MarkdownFileLink({
  ...
  const handleOpenInFilePreview = useCallback(() => {
    // ── Marcode fork seam ──
    // `onOpenInPanel` keeps upstream's project-index lookup for bare filenames
    // but lands the result in the floating Code editor, not the retired
    // right-panel file surface. The media and editor fallbacks are upstream's.
...
function ChatMarkdown({
  ...
  const openMarkdownMedia = useCallback(
    (source: string, resolvedFilePath?: string) => {
      ...
      void resolveMarkdownMediaPreview({
        ...
        createAssetUrl,
        // ── Marcode fork seam ── as above: the floating editor, not the
        // retired right-panel `file` surface.
  ...
  const openFileInPanel = useCallback(
    (panelPath: string, line: number | undefined) => {
      ...
      // ── Marcode fork seam ──
      // The floating Code editor is Marcode's only file-editing surface, so the
      // resolved path opens there rather than through the removed right-panel
      // `file` surface.
