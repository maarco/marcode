...
const MarkdownFileLink = memo(function MarkdownFileLink({
  ...
  const handleOpenInFilePreview = useCallback(() => {
    ...
    // Upstream's `onOpenInPanel` opened the right-panel file surface; Marcode
    // retired that surface and repoints the same callback at the floating
    // editor (see `openFileInPanel` below), so this call site is unchanged.
...
function ChatMarkdown({
  ...
  //
  // ── Marcode fork seam ──
  // Upstream opens the resolved path on the right panel's file surface. Marcode
  // retired that surface, so the same resolution feeds the floating editor. The
  // editor needs the environment-scoped identity, which is why this bails
  // without a `cwd` instead of falling back to a bare relative path.
  const openFileInPanel = useCallback(
    (workspaceRelativePath: string, line: number | undefined) => {
      ...
      if (!needsWorkspaceBasenameLookup(workspaceRelativePath)) {
