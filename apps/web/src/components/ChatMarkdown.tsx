...
// Marcode fork seam: upstream reads useRightPanelStore here to open file links in
// the right panel's file surface. Marcode retired that surface — file links open
// through the floating editor (openFileInPanel) — so the import stays dropped.
...
const MarkdownFileLink = memo(function MarkdownFileLink({
  ...
  const handleOpenInFilePreview = useCallback(() => {
    ...
    // ── Marcode fork seam ──
    // Upstream's `onOpenInPanel` resolves a bare filename through the project
    // index before opening; Marcode keeps that lookup and lands the result in
    // the floating editor instead of a right-panel file surface.
    onOpenInPanel(workspaceRelativePath, line);
  }, [handleOpenInEditor, line, onOpenInPanel, threadRef, workspacePath, workspaceRelativePath]);
