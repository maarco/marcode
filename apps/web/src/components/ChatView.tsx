...
function ChatViewContent(props: ChatViewProps) {
  ...
  // ── Marcode fork seam ──
  // Upstream derives an `activeFileSurface` here for the right panel's file view. Marcode retired
  // that surface (the floating editor owns file editing), so `RightPanelSurface` has no "file"
  // kind and nothing downstream consumes it.
  ...
  // ── Marcode fork seam ──
  // Upstream always renders this wrapper and puts `panelToggleControls` in it. Marcode moved the
  // standalone header toggles into the pill nav's workspace category, so only the maximize control
  // remains and the wrapper renders only while the panel is open. `workspace-titlebar-controls` in
  // index.css carries the positioning upstream inlines here.
          ...
          >
            ...
            >
              <div
                ...
                // `chat-composer-horizontal-inset` in index.css carries the same safe-area padding
                // upstream inlines here; Marcode adds the draft-hero centering on top.
                className={
                  isDraftHeroState
