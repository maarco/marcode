...
function ChatViewContent(props: ChatViewProps) {
  ...
  // ── Marcode fork seam ──
  // Upstream also builds an `openProjectPullRequest` here for its ChatHeader.
  // Marcode portals the thread-actions surface into FloatingPillNav and wires
  // `openThreadPullRequest` above, which is the same right-panel action and
  // additionally honours the thread's own linked pull request, so the
  // project-only variant has no caller here.
  ...
  return (
    <div className="relative flex min-h-0 min-w-0 flex-1 overflow-hidden bg-background">
      ...
      >
        ...
        >
          {/* ── Marcode fork seam ──
              Upstream threads project scripts, git actions, editors, keybindings
              and the pull-request action through ChatHeader. Marcode portals all
              of those into FloatingPillNav, so ChatHeader owns the title only. */}
