...
function ChatViewContent(props: ChatViewProps) {
  ...
  // No `activeFileSurface`: Marcode retired the right-panel file surface.
  ...
  // Upstream also renders `panelToggleControls` in this cluster. Marcode moved
  // the standalone header toggles into the pill nav's workspace category, so
  // only the maximize control remains and the cluster is dropped entirely when
  // the panel is closed rather than left as an empty box.
    ...
    rightPanelOpen && !shouldUseRightPanelSheet ? (
      <div
        className={cn(
          ...
          // sideways on every toggle. Upstream retired the
          // `.workspace-titlebar-controls` class for these utilities; the
          // data attribute is what the themed-control CSS now hooks.
          ...
          >
            ...
            >
              <div
                ...
                // Upstream retired the `.chat-composer-horizontal-inset` class
                // for these utilities. Marcode keeps the draft-hero variant,
                // which centers the composer column on the hero state.
                className={cn(
                  ...
                  isDraftHeroState && "flex flex-col items-center",
                )}
              >
