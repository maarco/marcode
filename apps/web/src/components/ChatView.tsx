...
function ChatViewContent(props: ChatViewProps) {
  ...
  // ── Marcode fork seam ──
  // Upstream also keeps an `openProjectPullRequest` that resolves only against
  // the active project, for its ChatHeader entry point. Marcode's single entry
  // point is ThreadActionsCluster in FloatingPillNav, which uses
  // `openThreadPullRequest` above — that one prefers the thread's linked pull
  // request and falls back to the project, so it subsumes the project-only
  // variant. Wire new upstream pull-request entry points into the cluster.
  ...
  return (
    <div className="relative flex min-h-0 min-w-0 flex-1 overflow-hidden bg-background">
      ...
      >
        ...
        >
          {/* ── Marcode fork seam ──
              Upstream's ChatHeader carries the thread actions (pull request,
              project scripts, new thread, editors) and renders the panel layout
              controls when the right panel is closed. Marcode portals all of
              that into FloatingPillNav and renders panelLayoutControls once
              above, so this header keeps only the thread title. Port upstream's
              new header entry points into FloatingPillNav, not back into here. */}
