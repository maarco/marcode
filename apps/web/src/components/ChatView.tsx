...
function ChatViewContent(props: ChatViewProps) {
  ...
  // ── Marcode fork seam ── upstream also derives an `activeFileSurface` here
  // from the right panel's `"file"` kind. Marcode retired that surface, so
  // there is nothing to derive and the binding is omitted.
        ...
        {pillThreadActionsSlot
          ? createPortal(
              <>
                ...
                {/* Upstream #8160 moved the git control's PR picker from the
                    thread's linked PR to the project's own repository. The
                    control lives in Marcode's pill-nav cluster, so the switch
                    is applied to `onOpenPullRequest` below. */}
                ...
                <div className="group/composer-stack pointer-events-auto relative z-10">
                  {isDraftHeroState ? (
                    <div className="relative z-0 w-full">
                      <div
                        className="px-4 pb-8 group-has-data-[composer-shoulder-tab]/composer-stack:pb-4 sm:px-6 sm:pb-10 sm:group-has-data-[composer-shoulder-tab]/composer-stack:pb-4"
                        style={
                  ...
                  >
                    {/* ── Marcode fork seam ── `chat-composer-glass-shell-transparent`
                        overrides `--glass-opacity`, which upstream's Shell still reads. */}
