...
import { MarcodeThreadActions } from "./chat/MarcodeThreadActions";
import {
...
export default function ChatView(props: ChatViewProps) {
  ...
  // The file-surface pending tracker retired with the right-panel file
  // surfaces; the floating Code editor owns file editing and its own state.
  ...
  // ── Marcode fork seam ──
  // Upstream previews a browser-viewable attachment in a right-panel `file`
  // surface. Marcode retired that surface and the floating editor opens
  // workspace paths, not attachment blobs, so every attachment downloads until
  // an attachment surface exists in the editor.
  ...
  const addDiffSurface = useCallback(() => {
    ...
    onDiffPanelOpen?.();
    // Upstream removed the plan sidebar (plans render inline in the
    // transcript), so its dismiss/open deps are gone with it.
  ...
  // No openFileSurface: upstream opens files in the right panel, Marcode opens
  // them in the floating Code editor (see editor/open-floating-file.ts).
  ...
  const rightPanelContent = activeThreadRef ? (
    renderedRightPanelSurface?.kind === "preview" ? (
      ...
      <PullRequestsUnavailableState
        ...
        error="Update this environment's Marcode server to browse pull requests."
      />
  ...
  const marcodeThreadActions = (
    <MarcodeThreadActions
      isDraftHeroState={isDraftHeroState}
      chatAmbientEffect={chatAmbientEffect}
      onChatAmbientEffectChange={handleChatAmbientEffectChange}
      chatAmbientAppearance={chatAmbientAppearance}
      onChatAmbientAppearanceChange={handleChatAmbientAppearanceChange}
      ...
      >
        ...
        >
          ...
          <ChatHeader
            ...
            onUpdateProjectScript={updateProjectScript}
            onDeleteProjectScript={deleteProjectScript}
            actions={pillThreadActionsSlot ? null : marcodeThreadActions}
          />
        </WorkspacePageHeader>

        {/* thread actions live in the pill nav's workspace slot */}
        {pillThreadActionsSlot ? createPortal(marcodeThreadActions, pillThreadActionsSlot) : null}
        <ThreadErrorBanner
                ...
                <div className="group/composer-stack pointer-events-auto relative z-10 mx-auto w-full max-w-3xl">
                  {isDraftHeroState ? (
                    <div className="relative z-0 w-full">
                      <div
                        className="px-4 pb-8 group-has-data-[composer-shoulder-tab]/composer-stack:pb-4 sm:px-6 sm:pb-10"
                        style={
                  ...
                  >
                    <ComposerSurface.Shell
                      className="chat-composer-glass-shell-transparent"
