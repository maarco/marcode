...
// Marcode uses the filled icon set for the composer's Open In control.
        ...
        <PopoverPopup align="center" className="p-1" side="bottom">
          <div aria-label="Open in editor" className="flex min-w-40 flex-col">
            {options.map(({ label, Icon, value, kind }) => (
              // No native `title` for the "no working directory yet" case: upstream's
              // no-native-title-tooltip rule bans it, and a disabled button does not
              // reliably fire the hover that would show it. The disabled state itself
              // carries the meaning; aria-label still names the action.
              <button
                key={value}
                type="button"
                disabled={!openInCwd}
