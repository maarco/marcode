...
// Marcode retired the right-panel file/files surfaces: the floating Code editor owns file
// editing. Upstream keeps shipping `onAddFiles`/`filesAvailable`/`onCopyFilePath` and the
// pending-file dot, and a merge re-adds them without conflicting. Fail here instead.
describe("RightPanelTabs file-surface removal", () => {
  it("exposes no file-surface props or pending-file badge", () => {
    const source = NodeFS.readFileSync(new URL("./RightPanelTabs.tsx", import.meta.url), "utf8");

    expect(source).not.toMatch(/\bonAddFiles\b/);
    expect(source).not.toMatch(/\bfilesAvailable\b/);
    expect(source).not.toMatch(/\bonCopyFilePath\b/);
    expect(source).not.toMatch(/\bpendingSurfaceIds\b/);
  });
});
