        ...
        >
          <div className="relative z-10 h-[4.875rem] px-[var(--sidebar-row-content-inset)] py-[var(--sidebar-content-inset)]">
            <div className="flex h-5 min-w-0 items-center gap-1.5">
              ...
              {/* 11px, a step under the 12px title register: status is metadata,
                  but a working label sits beside a spinner and should not
                  out-shout the thread title it belongs to. */}
              {sortable?.isDragging ? (
                dragDestination
              ) : (
...
export default function Sidebar() {
  ...
  const workspaceSectionProject = useMemo(() => {
    ...
    return {
      ...
      // Upstream's #10714 replaced the cwd-only map with the project record so
      // ProjectFavicon cannot drift; the tree only needs the workspace root.
      workspaceRoot:
        projectByKey.get(`${primaryRef.environmentId}:${primaryRef.projectId}`)?.workspaceRoot ??
        "",
    };
  }, [unifiedWorkspaceSidebarEnabled, scopedProjectGroup, projectByKey]);
