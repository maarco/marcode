...
// Keep the seam to this import plus the single mount below, so an upstream
// rewrite of Sidebar.tsx conflicts on two small hunks instead of the tree.
import { SidebarProjectWorkspaceSection } from "./unified-workspace/SidebarProjectWorkspaceSection";
// ─────────────────────────────────────────────────────────────────────────
        ...
        >
          <div className="relative z-10 h-[4.875rem] px-[var(--sidebar-row-content-inset)] py-[var(--sidebar-content-inset)]">
            <div className="flex h-5 min-w-0 items-center gap-1.5">
              ...
              {/* 11px, a step under the 12px title register: this slot is
                  metadata, but "Working" renders bold in the info colour next
                  to a spinner, so at an equal size it out-shouts the thread
                  title it belongs to. The relative-time fallback shares the
                  slot and so shares the size — otherwise a row would resize
                  its own corner the moment a run started. */}
              <span className="group/sidebar-status-slot relative ml-auto flex h-5 min-w-8 shrink-0 items-stretch justify-end text-[11px]">
                      ...
                      <Tooltip>
                        <TooltipTrigger
                          render={
                            ...
                            >
                              {/* size-3.5, not upstream's size-4: Marcode's sidebar status
                                  slot is 11px and size-4 overflows it. */}
...
export default function Sidebar() {
  ...
  // The tree is per-project, so it renders only with a project actually in
  // scope — "All projects" keeps upstream's plain thread list.
  const workspaceSectionProject = useMemo(() => {
    if (!unifiedWorkspaceSidebarEnabled || scopedProjectGroup === null) return null;
    const primaryRef = scopedProjectGroup.memberProjectRefs[0];
    if (!primaryRef) return null;
    return {
      environmentId: primaryRef.environmentId,
      projectId: primaryRef.projectId,
      displayName: scopedProjectGroup.displayName,
      workspaceRoot:
        projectCwdByKey.get(`${primaryRef.environmentId}:${primaryRef.projectId}`) ?? "",
    };
  }, [unifiedWorkspaceSidebarEnabled, scopedProjectGroup, projectCwdByKey]);
        ...
        <SidebarGroup className="ps-[calc(var(--sidebar-content-inset)+1px)] pe-[var(--sidebar-content-inset)] pb-1 pt-0">
          ...
          {!isSearchingThreads && workspaceSectionProject ? (
            <SidebarProjectWorkspaceSection
              ...
              projectDisplayName={workspaceSectionProject.displayName}
              workspaceRootFallback={workspaceSectionProject.workspaceRoot}
              isMobile={isMobile}
              ...
              {projects.length === 0 ? (
                <>
                  ...
                  <button
                    type="button"
                    onClick={openAddProjectCommandPalette}
                    className="inline-flex items-center gap-1.5 rounded-md border border-sidebar-border px-2.5 py-1 text-xs font-medium text-sidebar-muted-foreground transition-colors hover:bg-sidebar-row-hover hover:text-sidebar-foreground"
                  >
