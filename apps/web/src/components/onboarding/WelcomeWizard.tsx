...
export function WelcomeWizard({
  ...
  // Marcode fork seam: upstream dropped the onboarding theme surface and lets
  // the wizard follow the saved appearance. Marcode's wizard is always dark;
  // `mountOnboardingTheme` owns that canvas and restores the preference on
  // unmount. See apps/web/src/hooks/useTheme.ts.
  ...
  return (
    <Dialog open disablePointerDismissal onOpenChange={(_, event) => event.cancel()}>
      ...
      >
        <DialogTitle className="sr-only">Set up Marcode</DialogTitle>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="flex-1">Marcode Connect</span>
      ...
      <CollapsiblePanel keepMounted>
        <div className="px-3 pb-3">
          ...
          <p className="mt-3 text-xs text-muted-foreground">
            Keep Marcode running. Select the computers you want to set up above.
          </p>
        </div>
      </CollapsiblePanel>
    </Collapsible>
        ...
        <Collapsible>
          ...
          <CollapsiblePanel className="pt-3">
            ...
            <p className="mt-2 text-xs leading-relaxed text-muted-foreground">
              Start Marcode first, or run <code className="font-mono">npx t3 serve</code>. Add{" "}
      ...
      <div className="h-64">
        {terminalReady ? (
          <TerminalViewport
            ...
            // Marcode's viewport only attaches to an existing terminal — this
            // wizard already opened one with `providerInstanceId` above — and it
            // requires a chat-context sink the onboarding flow has no use for.
