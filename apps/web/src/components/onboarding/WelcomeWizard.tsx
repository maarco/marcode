...
// ── Step 2: Marcode Connect (sign in, then connect machines) ──────
...
function ConnectMachinesStep({
  ...
  if (!isLoaded) {
    return <StepShell title="Sign in to Marcode Connect" onBack={onBack} />;
  }

  if (!isSignedIn) {
    return (
      <StepShell
        title="Sign in to Marcode Connect"
          ...
          <Collapsible className="mt-4">
            ...
            <CollapsiblePanel>
              ...
              <p className="mt-2 text-xs text-muted-foreground">
                Keep Marcode running on that computer. If it is not running, open Marcode or run{" "}
