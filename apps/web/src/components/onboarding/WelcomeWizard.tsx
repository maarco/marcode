...
export function WelcomeWizard({
  ...
  return (
    <Dialog open disablePointerDismissal onOpenChange={(_, event) => event.cancel()}>
      ...
      >
        <DialogTitle className="sr-only">Set up {APP_DISPLAY_NAME}</DialogTitle>
        <div className="flex min-h-0 flex-col">
          <DialogHeader className="gap-4">
            {/* ── Marcode fork seam ── upstream renders the T3 wordmark plus
                "Code" here; Marcode's product identity is a text mark, the same
                one the pairing and auth surfaces use. */}
      ...
      <div className="h-64">
        {terminalReady ? (
          <TerminalViewport
            ...
            // ── Marcode fork seam ── Marcode's viewport only attaches to an
            // existing terminal — this wizard already opened one with
            // `providerInstanceId` above — and it requires a chat-context sink
            // the onboarding flow has no use for.
