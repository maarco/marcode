...
export function WelcomeWizard({
  ...
  return (
    <Dialog open disablePointerDismissal onOpenChange={(_, event) => event.cancel()}>
      ...
      >
        <WizardHeader
          title="Set up Marcode"
          identity={
            // ── Marcode fork seam ── upstream renders their T3 wordmark plus a
            // "Code" lockup here; Marcode's identity is the MarcodeMark glyph
            // and the product name from branding.ts.
            <div className="flex items-baseline gap-1.5" role="img" aria-label={APP_BASE_NAME}>
              <MarcodeMark className="h-5 w-5 shrink-0" />
