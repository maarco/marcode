...
function SurfaceIcon({
  ...
  sessions: Readonly<Record<string, PreviewSessionSnapshot>>;
  desktopByTabId: Readonly<Record<string, DesktopPreviewOverlay>>;
  // No `theme`: upstream threads it in only to tint PierreEntryIcon for the
  // retired `file` surface. `desktopByTabId` is a different feature (captured
  // browser-tab favicons) and is threaded through.
                ...
                <MenuTrigger
                  render={
                    <Button
                      ...
                      className="relative size-6 shrink-0 cursor-pointer text-muted-foreground hover:bg-accent hover:text-foreground"
                      size="icon-xs"
                      variant="ghost"
                    />
                  }
