...
// No `Files`: upstream's `files` right-panel surface is retired in Marcode.
import {
  Bot,
  FileDiff,
  GitPullRequest,
...
// No `PierreEntryIcon`: upstream imports it only for the retired `file` surface icon.
...
export function RightPanelTabs(props: RightPanelTabsProps) {
  ...
  const addSurfaceActions = [
    ...
    // No "Files" entry: upstream offers a right-panel files surface here, which
    // Marcode retired in favor of the floating Code editor. Shortcut "F" stays
    // free rather than being reassigned, so upstream's row merges back cleanly.
    {
      label: "Diff",
      icon: FileDiff,
      shortcut: "D",
  ...
  const handleTabContextMenu = useCallback(
    async (event: ReactMouseEvent, surface: RightPanelSurface) => {
      ...
      // No "Copy path": upstream offers it for right-panel file tabs, which
      // Marcode retired in favor of the floating Code editor.
            ...
            {props.surfaces.map((surface) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    {/* Upstream badges a "pending" dot here, fed by pending
                        FILE surfaces. Marcode retired the file surfaces, so
                        there is nothing to be pending and the dot is dropped
                        rather than wired to a always-empty set. */}
