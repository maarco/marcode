...
export function TerminalViewport({
  ...
  useEffect(() => {
    ...
    // ── Marcode fork seam ──
    // Upstream's terminal is the Ghostty WASM surface, whose
    // `pasteFromClipboard(read, isCurrent)` claims the paste race inside the
    // surface. Marcode keeps xterm (for its search addon), which has no such
    // hook, so the same race is settled here: re-check the request id after
    // the clipboard read resolves and drop a paste that a newer flow
    // superseded, rather than writing stale text into the pty.
    const pasteFromClipboard = async (requestId: number) => {
      ...
      if (requestId !== selectionActionRequestIdRef.current) return;
      if (text.length > 0) {
        await sendTerminalInput(text, "Unable to paste into the terminal");
      }
      focusIfCurrent(requestId);
    };
    ...
    // Upstream hands this to the Ghostty surface as an `onContextMenu` option.
    // xterm has no equivalent hook, so the same handler is bound to the mount
    // element here.
