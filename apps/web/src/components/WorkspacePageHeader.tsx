...
export function WorkspacePageHeader({
  ...
  /**
   * ── Marcode fork seam ──
   * Settings and Usage never mount a sidebar. The collapsed-sidebar inset keys
   * off a `data-sidebar-state` ancestor, so on those routes it never matches
   * and the breadcrumb slides under the native window controls. Pinned by
   * `workspaceTitlebar.fork.test.ts`.
   */
