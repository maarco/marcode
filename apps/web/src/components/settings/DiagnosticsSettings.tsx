        ...
        if (!result.value.signaled) {
          ...
          if (isStaleProcessSignalMessage(message)) {
            toastManager.add({
              ...
              description:
                "The process is not a child of the Marcode Server. It might already have exited.",
            });
            return;
          }
      ...
      >
        <StatsGrid>
          <StatBlock
            ...
            tooltip="Approximate active CPU time for the Marcode server root process and its descendants during the selected window. It grows only while sampled processes use CPU and older samples leave as the window moves."
          />
          <StatBlock
            label="Samples"
