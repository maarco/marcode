...
const isSingleOriginDev = process.env.MARCODE_SINGLE_ORIGIN_DEV === "1";
...
const configuredRelayUrl = repoEnv.VITE_MARCODE_RELAY_URL?.trim() || "";
...
// as hot patches. Opt-in while experimental: MARCODE_BUNDLED_DEV=1 pnpm dev:web
// The dev runner defaults this on for --share runs (remote browsers pay a
// round trip per import level in unbundled dev); MARCODE_BUNDLED_DEV=0 opts out.
const bundledDevEnv = process.env.MARCODE_BUNDLED_DEV?.trim().toLowerCase();
...
function resolveDevProxyTarget(
  ...
  // MARCODE_PORT is set by scripts/dev-runner.ts for every non-desktop mode.
...
const devProxyTarget = resolveDevProxyTarget(process.env.MARCODE_PORT, configuredWsUrl);
...
const configuredAllowedHosts = (process.env.MARCODE_DEV_ALLOWED_HOSTS ?? "")
  .split(",")
...
export default defineConfig(() => {
  return {
    ...
    define: {
      ...
      "import.meta.env.VITE_MARCODE_RELAY_URL": JSON.stringify(configuredRelayUrl),
