...
const makeEnvironment = (overrides: Record<string, unknown> = {}) =>
  DesktopEnvironment.DesktopEnvironment.of({
    ...
    linuxDesktopEntryName: "marcode.desktop",
    linuxWmClass: "t3code",
...
describe("DesktopLinuxUrlHandler", () => {
  ...
  it("carries structured context on registration errors", () => {
    const writeError = new DesktopLinuxUrlHandler.DesktopLinuxUrlHandlerRegistrationError({
      ...
      scheme: "t3code",
      desktopEntryPath: "/home/alice/.local/share/applications/marcode.desktop",
    ...
    assert.equal(
      ...
      "/home/alice/.local/share/applications/marcode.desktop",
    );
  ...
  it.effect("writes the handler entry and claims the scheme default via xdg-mime", () => {
    ...
    return Effect.gen(function* () {
      ...
      // The desktop-entry FILENAME comes from the fixture's
      // linuxDesktopEntryName, but the scheme comes from
      // ElectronProtocol.getDesktopScheme() — the real protocol this build
      // registers, which is `marcode` here. Upstream's copy of this test
      // asserts `t3code` for both.
      assert.include(recorded.files[0]?.content, "MimeType=x-scheme-handler/marcode;");
      assert.deepEqual(recorded.commands, [
        {
          command: "xdg-mime",
          args: ["default", "marcode.desktop", "x-scheme-handler/marcode"],
        },
      ]);
    });
  });
    ...
    return Effect.gen(function* () {
      yield* runRegister(recorded, {
        existingEntry: DesktopLinuxUrlHandler.renderUrlHandlerDesktopEntry({
          ...
          // ── Marcode fork seam ── the real scheme this build registers.
          scheme: "marcode",
        }),
      });
  ...
  it.effect("writes the portal identity without claiming the URL scheme in development", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.equal(
        ...
        "/home/alice/.local/share/applications/marcode-dev.desktop",
      );
    ...
    return Effect.gen(function* () {
      ...
      yield* runRegister(writeFailed, {
        writeError: PlatformError.systemError({
          ...
          pathOrDescriptor: "/home/alice/.local/share/applications/marcode.desktop",
        }),
      });
