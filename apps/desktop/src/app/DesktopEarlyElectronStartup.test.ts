...
describe("DesktopEarlyElectronStartup", () => {
  ...
  it("reads the persisted linux password-store preference before Electron is ready", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: { MARCODE_HOME: "/home/user/.t3-test" },
  ...
  it("resolves the early linux Electron switches", () => {
    const options = resolveEarlyLinuxElectronOptions({
      env: {
        MARCODE_HOME: "/home/user/.t3-test",
  ...
  it("keeps implicit development state under ~/.marcode/dev when MARCODE_HOME is unset", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: {
        VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
      },
      ...
      joinPath,
      readFileString: (path) => {
        assert.equal(path, "/home/user/.marcode/dev/desktop-settings.json");
  ...
  it("treats whitespace-only MARCODE_HOME as unconfigured in development", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: {
        MARCODE_HOME: "   ",
        VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
