...
const environmentLayer = (input: {
  ...
  DesktopEnvironment.layer({
    ...
    Layer.provide(
      Layer.mergeAll(
        NodeServices.layer,
        // MARCODE_HOME / MARCODE_MODE, not upstream's T3CODE_* names: DesktopConfig
        // reads the Marcode-prefixed variables, so the T3CODE_ spelling leaves
        // `marcodeHome` unset and the state dir resolves away from tempDir.
