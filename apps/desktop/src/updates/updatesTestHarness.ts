...
export function makeHarness(options: UpdatesHarnessOptions = {}) {
  ...
  const environmentLayer = DesktopEnvironment.layer({
    ...
    Layer.provide(
      Layer.mergeAll(
        NodeServices.layer,
        // Marcode fork seam: `DesktopConfig` reads MARCODE_* env names, so an
        // upstream harness that keeps T3CODE_* silently drops every setting
        // here and the update tests assert against defaults instead.
        DesktopConfig.layerTest({
          MARCODE_HOME: `/tmp/t3-desktop-updates-test-${process.pid}`,
          MARCODE_DESKTOP_MOCK_UPDATES: "true",
          MARCODE_DESKTOP_MOCK_UPDATE_SERVER_PORT: "4141",
          ...options.env,
        }),
      ),
    ),
