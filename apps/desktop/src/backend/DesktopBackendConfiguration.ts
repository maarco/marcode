...
const DESKTOP_BACKEND_ENV_NAMES = [
  ...
  "MARCODE_NO_BROWSER",
  "MARCODE_HOST",
  "MARCODE_DESKTOP_WS_URL",
  "MARCODE_DESKTOP_LAN_ACCESS",
  "MARCODE_DESKTOP_LAN_HOST",
  "MARCODE_DESKTOP_HTTPS_ENDPOINTS",
  "MARCODE_TAILSCALE_SERVE",
  "MARCODE_TAILSCALE_SERVE_PORT",
] as const;
...
const resolvePrimaryStartConfig = Effect.fn("desktop.backendConfiguration.resolvePrimary")(
  ...
  > {
    ...
    const bootstrap = {
      ...
      marcodeHome: environment.baseDir,
    ...
    return {
      ...
      // Primary wants process.env (PATH, dev-runner's MARCODE_HOME, etc.).
      extendEnv: true,
      bootstrap,
...
> {
  ...
  const bootstrap = {
    ...
    noBrowser: true,
    port: input.port,
    // Omit marcodeHome so the Linux backend uses its own home dir instead of
  ...
  // Build an explicit copy of process.env minus MARCODE_HOME (dev-runner
  // exports the Windows-side base dir for the primary; if it leaks into
  // the WSL backend the Linux side ends up sharing C:\Users\...\.marcode via
  // /mnt/c, which means both backends read/write the same database and
  // their env-ids collide).
  const parentEnvWithoutMarcodeHome: Record<string, string | undefined> = {};
  for (const [key, value] of Object.entries(process.env)) {
    if (key === MARCODE_HOME_ENV) continue;
    parentEnvWithoutMarcodeHome[key] = value;
  }
  const wslEnv = mergeWslEnv(parentEnvWithoutMarcodeHome.WSLENV, forwardedEnvNames);
  ...
  const baseConfig = {
    ...
    cwd: environment.backendCwd,
    env: {
      ...parentEnvWithoutMarcodeHome,
      ...backendChildEnvPatch(),
      ...forwardedEnv,
      ...(wslEnv !== undefined ? { WSLENV: wslEnv } : {}),
    },
    // env is already a complete process.env minus MARCODE_HOME; pass it
