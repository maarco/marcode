...
const devBundleIdSuffix = NodePath.basename(repoRoot)
  .toLowerCase()
  .replaceAll(/[^a-z0-9]+/g, "");
// ── Marcode fork seam ── macOS bundle identity and URL schemes are Marcode-owned.
// LAUNCHER_VERSION follows upstream so their launcher-cache busts stay effective.
...
function buildMacLauncher(electronBinaryPath) {
  ...
  if (isDevelopment) {
    ...
    // Electron.app even though this bundle's Info.plist has the Marcode name.
