...
The helper mints the credential with Marcode's port variable. If you have to run the step by hand — on PowerShell, for example, where the bash helper does not run — set `$env:MARCODE_PORT = "<server-port>"` and call `node apps/server/src/bin.ts auth pairing create --base-dir <base-dir> --base-url <mobile-origin> --ttl 15m --label agent-mobile-<short-device-id>` yourself.
