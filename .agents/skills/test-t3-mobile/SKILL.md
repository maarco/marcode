...
Run only the command for the selected platform. The helper uses `http://127.0.0.1:<server-port>` for iOS and `http://10.0.2.2:<server-port>` for Android. Pass a fifth argument only when testing a non-development URL scheme; the default is Marcode's `marcode-dev` scheme.

The helper is POSIX shell. Where it cannot run (PowerShell without bash), mint the credential by hand — the port variable is `MARCODE_PORT`, not the upstream `T3CODE_PORT`:

```powershell
$env:MARCODE_PORT = "<server-port>"
node apps/server/src/bin.ts auth pairing create --base-dir <base-dir> --base-url <mobile-origin> --ttl 15m --label agent-mobile-<short-device-id>
```
