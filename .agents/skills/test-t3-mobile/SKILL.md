...
The helper needs a POSIX shell. On Windows, run it from Git Bash or WSL, or mint the credential directly with `$env:MARCODE_PORT = "<server-port>"` followed by `node apps/server/src/bin.ts auth pairing create --base-dir <base-dir> --base-url <mobile-origin> --ttl 15m --label agent-mobile-<short-device-id>`.
